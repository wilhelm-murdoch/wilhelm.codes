...
| Icons          | Self-hosted SVGs (Phosphor duotone, Octicons, Simple Icons), inlined at build time |
...
Icons are committed SVGs under `assets/icons/` and inlined by [layouts/partials/icon.html](layouts/partials/icon.html) — no icon font or runtime JS. Regenerate them with [`scripts/fetch-icons.sh`](scripts/fetch-icons.sh); sources and licenses are listed in [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md).
...
assets/       Tailwind entry (css/) and inlined icon SVGs (icons/), processed by Hugo
...
- `pack` + `icon` — icon family and name (e.g. `pack: duotone`, `icon: code`). `pack: brands` resolves to `assets/icons/brands/`, anything else to `assets/icons/ui/`; `icon` is the SVG filename (without extension).
...
Prettier is scoped by [.prettierignore](.prettierignore) to templates and config only — it never touches authored content, data fixtures, build output, or the committed icon SVGs.
